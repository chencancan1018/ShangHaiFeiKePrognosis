...
if __name__ == "__main__":
    ...
    existed_path = '/home/tx-deepocean/workspace_ccc/Project/ShangHaiFeiKe_prognosis/preprocess/predata_level2_margin_tdot9_sttd' 

    patch_dir = "/home/tx-deepocean/workspace_ccc/Project/ShangHaiFeiKe_prognosis/preprocess/predata_level2_margin_tdot9"
    out_dir = existed_path
    # pids = os.listdir(patch_dir)
    # pids = sorted([str(p) for p in os.listdir(patch_dir) if p.startswith('13')])
    # pids = sorted([str(p) for p in os.listdir(patch_dir) if p.startswith('13')])[:55]
    pids = sorted([str(p) for p in os.listdir(patch_dir) if p.startswith('13')])[55:]
    ...
    for p in pids:
        ...
        patch_folders = [f for f in patch_folders if not f.endswith('sttd')]
